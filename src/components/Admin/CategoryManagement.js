    ...
    try {
      ...
      const isNameDuplicate =
        ...
        querySnapshot.docs.some(
          (doc) => doc.id !== currentCategoryId && doc.data().name === categoryForm.name
        );
  
      const isSlugDuplicate =
        !currentCategoryId ||
        slugSnapshot.docs.some(
          (doc) => doc.id !== currentCategoryId && doc.data().slug === categoryForm.slug
        );
  
      if (isNameDuplicate || isSlugDuplicate) {
        alert(
          `Tên thể loại hoặc Slug đã tồn tại.`
        );
        return;
      }
